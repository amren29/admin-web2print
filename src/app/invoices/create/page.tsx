...
export default function InvoiceBuilderPage() {
    ...
    const applyCoupon = async () => {
        ...
        try {
            ...
            if (res.ok) {
                setAppliedDiscount({ code: data.couponCode, amount: data.discountAmount });
    ...
    const handleFinalizeInvoice = async () => {
        ...
        try {
            const payload = {
                ...
                phone: customerPhone,
                items: cart,
                totalAmount: cartSubtotal - (appliedDiscount?.amount || 0),
        ...
        <div className="flex h-[calc(100vh-theme(spacing.16))] gap-6 p-6">
            ...
            <div className="flex-1 w-2/3 flex flex-col space-y-4">
                <div className="flex items-center gap-4">
                    <h1 className="text-xl font-bold bg-purple-100 text-purple-700 px-3 py-1 rounded">New Invoice</h1>
            ...
            <div className="w-1/3 min-w-[350px] bg-white border rounded-xl shadow-lg flex flex-col overflow-hidden h-full">
                <div className="p-4 bg-gray-50 border-b flex items-center justify-between">
                    ...
                    <Badge variant="secondary" className="bg-purple-100 text-purple-700">{cart.length} items</Badge>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    {cart.length === 0 ? (
                        ...
                        cart.map((item, idx) => (
                            <div key={item.id} className="group relative border rounded-lg p-3 hover:border-purple-300 transition-colors bg-white">
                                <Button
                ...
                <div className="p-6 bg-gray-50 border-t space-y-4">
                    ...
                    <div className="space-y-2">
                        <Label className="text-xs text-gray-500 font-bold uppercase">Discount Code</Label>
                        <div className="flex gap-2">
                            ...
                            />
                            {appliedDiscount ? (
                                <Button size="sm" variant="outline" className="h-9 text-red-600 hover:text-red-700" onClick={() => { setAppliedDiscount(null); setCouponCode(""); }}>
                                    Remove
                                </Button>
                            ) : (
                                <Button size="sm" variant="secondary" className="h-9" onClick={applyCoupon} disabled={!couponCode || cart.length === 0 || checkingCoupon}>
                                    {checkingCoupon ? <Loader2 size={14} className="animate-spin" /> : "Apply"}
                                </Button>
                            )}
                        </div>
                        {appliedDiscount && (
                            <div className="flex justify-between items-center text-sm text-green-600 bg-green-50 px-2 py-1 rounded border border-green-200">
                                <span>Coupon: <b>{appliedDiscount.code}</b> applied</span>
                                <span>-RM{appliedDiscount.amount.toFixed(2)}</span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="flex justify-between items-center text-lg font-bold text-gray-900 pt-2 border-t mt-2">
                            <span>Total Due</span>
                            <span>RM{(cartSubtotal - (appliedDiscount?.amount || 0)).toFixed(2)}</span>
                        </div>
                    </div>

                    <Button
                        size="lg"
                        className="w-full font-bold shadow-md bg-purple-600 hover:bg-purple-700"
            ...
            <Dialog open={isFinalizeOpen} onOpenChange={setIsFinalizeOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>Enter customer details for this invoice.</DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleFinalizeInvoice} disabled={isSaving}>
